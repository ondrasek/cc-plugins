...
TOOL_HINTS=(
    ...
    [coverage]="Run 'uv run pytest --cov=src/ --cov-report=term-missing' to see which lines are uncovered. Add tests for the uncovered code paths."
...
# Sync once upfront so all tools are available, then use --no-sync on each
# uv run call to avoid re-resolving (prevents race conditions with concurrent
# uv processes that cause intermittent "Failed to spawn" errors).
...
run_check        "coverage"       uv run --no-sync pytest --cov=src/ --cov-report=term --cov-fail-under=80 -q
...
run_check        "mypy"           uv run --no-sync mypy src/
run_check        "bandit"         uv run --no-sync bandit -r src/ -q -ll
run_check_nonempty "vulture"        uv run --no-sync vulture src/ --min-confidence 80
run_check        "xenon"          uv run --no-sync xenon --max-absolute B --max-modules A --max-average A src/
run_check_nonempty "refurb"         uv run --no-sync refurb src/ --python-version 3.13
run_check        "import-linter"  uv run --no-sync lint-imports
run_check_nonempty "semgrep"        uv run --no-sync semgrep scan --config p/python --error --quiet src/
run_check        "ty"             uv run --no-sync ty check src/
run_check        "interrogate"    uv run --no-sync interrogate src/ -v --fail-under 70 -e tests/
